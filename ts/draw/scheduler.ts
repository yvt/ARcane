...
export interface RenderOperation<T>
{
    name: string;
...
export interface RenderBufferInfos<T>
{
    [key: string]: RenderBufferInfo<T>;
}
export interface RenderBuffers
{
    [key: string]: RenderBuffer | null;
}
...
export function preprocessRenderOperations<T>(ops: RenderOperation<T>[]): RenderOperationRaw<T>[]
{
    return ops.map((op) => {
        const inKeys: string[] = [];
...
interface RenderOperationRaw<T>
{
    name: string;
    ...
    /** input buffers; must be distinct from each other and outputs. */
    inputs: RenderBufferInfo<T>[];

    /** output buffers; must be distinct from each other and inputs.
        */
    outputs: RenderBufferInfo<T>[];
...
/** Information needed to realize RenderBuffer.
    ...
    */
export class RenderBufferInfo<T>
{
    ...
    canMergeWith(o: RenderBufferInfo<T>): boolean
    {
        return this == o;
    }
    create(manager: RenderPipeline<T>): RenderBuffer
    {
...
class RenderBufferInfoMap<S, T> extends Map<RenderBufferInfo<S>, T>
{
    computeHash(key: RenderBufferInfo<S>): number
    {
        return key.hash;
    }

    equals(key1: RenderBufferInfo<S>, key2: RenderBufferInfo<S>): boolean
    {
        return key1.canMergeWith(key2);
    }
}

interface RenderBufferAllocation<T>
{
    info: RenderBufferInfo<T>;
    index: number;

    // used in RenderBufferAllocationMap
    next: RenderBufferAllocation<T> | null;
}
...
class RenderBufferAllocationMap<T>
{
    maxNumAllocated: number;
    numAllocated: number;
    private firstFree: RenderBufferAllocation<T> | null;

    constructor(public info: RenderBufferInfo<T>)
    {
        this.maxNumAllocated = 0;
        this.numAllocated = 0;
        this.firstFree = null;
    ...
    release(allocation: RenderBufferAllocation<T>): void
    {
...
class RenderBufferInfoAllocationEstimationMap<T> extends RenderBufferInfoMap<T, RenderBufferAllocationEstimationMap>
{
    getOrCreate(key: RenderBufferInfo<T>): RenderBufferAllocationEstimationMap
    {
...
class RenderBufferInfoAllocationMap<T> extends RenderBufferInfoMap<T, RenderBufferAllocationMap<T>>
{
    getOrCreate(key: RenderBufferInfo<T>): RenderBufferAllocationMap<T>
    {
        let ret = this.get(key, null);
        if (ret == null) {
...
interface RenderBufferBinding<T>
{
    info: RenderBufferInfo<T>;
    allocated: RenderBufferAllocation<T>;
}

interface RenderBufferBindingWithWritability<T>
{
    binding: RenderBufferBinding<T>;

    useCount: number;
}

interface RenderPhase<T>
{
    operation: RenderOperationRaw<T>;
    inputs: RenderBufferAllocation<T>[];
    outputs: (RenderBufferAllocation<T> | null)[];
}
...
export function dumpRenderOperationAsDot<T>(raw: RenderOperation<T>[]): string
{
    // alloc variable index
    let addedInfos: boolean[] = [];
    let infos: RenderBufferInfo<T>[] = [];
    {
        let index = 0;
    ...
    function writeRBI(rb: RenderBufferInfo<T>)
    {
...
export class RenderPipeline<T>
{
    private renderBuffers: RenderBufferInfoMap<T, RealizedRenderBufferGroup>;
    private phases: RenderPhase<T>[] = [];
    ...
    constructor(logger: Logger, profiler: Profiler, public readonly context: T)
    {
        this.logger = logger;
        this.profiler = profiler;

        this.renderBuffers = new RenderBufferInfoMap<T, RealizedRenderBufferGroup>();
    }
    ...
    setup(preprocOpsRaw: RenderOperation<T>[], output: RenderBufferInfo<T>[]): void
    {
        ...
        let ops: RenderOperationRaw<T>[] = [];
        {
            let needed: RenderBufferInfo<T>[] = [];
            let neededMap: (RenderBufferInfo<T> | null)[] = [];
            for (const info of output) {
                needed.push(info);
                neededMap[info.num] = info;
            }
            while (needed.length > 0) {
                const newNeededMap: (RenderBufferInfo<T> | null)[] = [];
                ...
                const newNeeded: RenderBufferInfo<T>[] = [];
                for (const key in newNeededMap) {
                    const info = newNeededMap[key];
                    if (info)
                        newNeeded.push(info);
                }
                needed = newNeeded;
        ...
        const needed: RenderBufferBindingWithWritability<T>[] = [];
        const phases: RenderPhase<T>[] = [];
        const outputAllocation: RenderBufferBinding<T>[] = [];
        ...
        while (needed.length > 0) {
...

            {
                ...
                for (let k = 0; k < op.inputs.length; ++k) {
                    ...
                    if (bound >= 0) {
                        ...
                        if (bound >= 0) {
                            ...
                            const alloc = phase.outputs[bound];
                            if (!alloc) {
                                throw new Error();
                            }
                ...
                for (let j = 0; j < op.outputs.length; ++j) {
                    ...
                    if (bound < 0) {
                        // not bound to output
                        const alloc = phase.outputs[j];
                        if (alloc) {
                            allocMap.getOrCreate(alloc.info).release(alloc);
                        }
                    }
                }
        ...
        const newRenderBuffers = new RenderBufferInfoMap<T, RealizedRenderBufferGroup>();
        allocMap.forEach((info, allocMapEntry) => {
            const existingRenderBufferEntry = this.renderBuffers.get(info, null);
        ...
        this.operators = phases.map((phase) =>
            phase.operation.factory({
                inputs: phase.inputs.map((input) => newRenderBuffers.getUnwrap(input.info).renderBuffers[input.index]),
                outputs: phase.outputs.map((output) => output ? newRenderBuffers.getUnwrap(output.info).renderBuffers[output.index] : null)
            }));
        this.outputBuffers = outputAllocation.map((alloc) => newRenderBuffers.getUnwrap(alloc.info).renderBuffers[alloc.allocated.index]);
