...
(function() {
	...
	// Let's just use the pre-built ARToolKit v5 module that comes with the `jsartoolkit` NPM package.
	// This loads into the global scope. Also note that this module modifies `window.artoolkit` by
	// itself.
	require('script-loader!jsartoolkit/build/artoolkit.min.js');
	...
	*/
	ARController.getUserMediaARController = function(configuration) {
		...
		obj.onSuccess = function(video, stream) {
			new ARCameraParam(cameraParamURL, function() {
				...
				onSuccess(arController, arCameraParam, stream);
			}, function(err) {
				console.error("ARController: Failed to load ARCameraParam", err);
			});
		};
	...
	*/
	...
	/* Exports */
	window.artoolkit = module.exports.artoolkit = artoolkit;
	module.exports.ARController = ARController;
	module.exports.ARCameraParam = ARCameraParam;
