...
export class Map<TKey, TValue>
{
    ...
    private first: MapNode<TKey, TValue> | null;
    ...
    get<T>(key: TKey, defaultValue: T): TValue | T
    {
        const hash = this.computeHash(key);
        const node = this.table[hash];
        if (node) {
            let item: MapItem<TKey, TValue> | null = node.head;
            while (item != null) {
    ...
    getUnwrap(key: TKey): TValue
    {
        const x = this.get(key, NoVal);
        if (x === NoVal) {
            throw new Error();
        }
        return <any> x;
    ...
    remove(key: TKey): boolean
    {
        ...
        let node = this.table[hash];
        if (node) {
            let prevItem: MapItem<TKey, TValue> | null = null;
            let item: MapItem<TKey, TValue> | null = node.head;
            while (item != null) {
                if (this.equals(key, item.key)) {
                    if (prevItem) {
    ...
    forEach(cb: (key: TKey, value: TValue) => void): void
    {
        if (this.first) {
            ...
            do {
                ...
                let item: MapItem<TKey, TValue> | null = node.head;
                do {
                    const nextItem: MapItem<TKey, TValue> | null = item.next;
